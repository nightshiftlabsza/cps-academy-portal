...
let skill='',dateFrom='',dateTo='',owner='';
let db={},tab='Home',query='',searchPage=0,filter='All',facet='',sort='source',page=0,mode='cards',selected=null,editingTab='',workspace={edits:{},added:[],favorites:[],history:[],recent:[]},storageIssue=false;
let initialFormValues={};
try{const saved=JSON.parse(localStorage.getItem(KEY)||'null');if(saved&&saved.edits&&Array.isArray(saved.added)&&Array.isArray(saved.favorites)&&Array.isArray(saved.history)){workspace=saved;if(!Array.isArray(workspace.recent))workspace.recent=[];}else workspace.edits=JSON.parse(localStorage.getItem('cps-workbook-edits-v1')||'{}')||{}}catch{storageIssue=true}
...
function navigate(t){tab=t;query='';searchPage=0;filter=t==='Morning Report'?'Upcoming':'All';owner='';facet='';skill='';dateFrom='';dateTo='';sort=t==='CPS Academy VMRs'?'date':'source';page=0;if(t==='Morning Report')mode='agenda';else if(['Podcast Episodes','Schema review'].includes(t))mode=mode==='cards'||mode==='table'?mode:'board';else if(mode==='agenda'||mode==='board')mode='cards';$('#global-search').value='';location.hash=encodeURIComponent(t);render();window.scrollTo(0,0)}
...
function currentLeader(){const list=records('Leader of the Week');const now=today();for(const r of list){const raw=r.fields.Dates||'';const m=raw.match(/^(\d{1,2})\/(\d{1,2})\s*-\s*(\d{1,2})\/(\d{1,2})$/);if(m){const sm=m[1].padStart(2,'0'),sd=m[2].padStart(2,'0'),em=m[3].padStart(2,'0'),ed=m[4].padStart(2,'0');const sYear=sm==='12'&&em==='01'?'2025':'2026',eYear='2026';const start=`${sYear}-${sm}-${sd}`,end=`${eYear}-${em}-${ed}`;if(now>=start&&now<=end)return r;}const isoM=raw.match(/^(\d{4}-\d{2}-\d{2})\s*(?:-|to)\s*(\d{4}-\d{2}-\d{2})$/);if(isoM&&now>=isoM[1]&&now<=isoM[2])return r;}return null;}
...
function home(){
 const upcoming=records('Morning Report').filter(r=>iso(dateValue(r))&&dateValue(r)>=today()).sort((a,b)=>dateValue(a).localeCompare(dateValue(b)));
 ...
 if(nextSession){
  ...
  const pres=nf.Presenter||(nf['Scribe / teaching points sign-ups']||'').match(/Case Presenter:\s*([^\n]+)/i)?.[1]?.trim()||'Unassigned';
...
function bindStaffingTools(targetRole){if(!$('#assign-name'))return;if(targetRole&&$('#staff-role')){$('#staff-role').value=targetRole;setTimeout(()=>$('#staff-name')?.focus(),60)}$('#assign-name').onclick=()=>{const name=$('#staff-name').value.trim(),role=$('#staff-role').value;if(!name){$('#staff-message').textContent='Enter a name first.';return}if(role==='Scribe'||role==='Teaching Points'){const el=[...$('#dialog-content').querySelectorAll('[data-field]')].find(el=>el.dataset.field==='Scribe / teaching points sign-ups');if(el){const pattern=new RegExp(`(${role}:\\s*)([^\\n]*)`,'i');if(pattern.test(el.value)){el.value=el.value.replace(pattern,`$1${name}`)}else{el.value=(el.value?el.value+'\n':'')+`${role}: ${name}`}el.dispatchEvent(new Event('input',{bubbles:true}));$('#staff-message').textContent=`${role} filled. Save changes to keep the assignment.`;el.focus();return}}const el=[...$('#dialog-content').querySelectorAll('[data-field]')].find(el=>el.dataset.field===role);if(!el)return;const current=el.value.trim();if(current&&!/^(tbd|none|-)$/i.test(current)){if(current.split(/[,;\n]/).some(s=>s.trim().toLowerCase()===name.toLowerCase())){$('#staff-message').textContent='That name is already assigned.';return}el.value=current+', '+name}else el.value=name;el.dispatchEvent(new Event('input',{bubbles:true}));$('#staff-message').textContent=`${role} filled. Save changes to keep the assignment.`;el.focus()}}

function mrGaps(r){const f=r.fields,fac=(f.Facilitator||'').trim();if(/canceled|cancelled/i.test(fac)||fac.toLowerCase()==='none'||(f.Type||'').trim().toLowerCase()==='none')return[];const gaps=[];if(!fac||/^(tbd|none|-)$/i.test(fac))gaps.push('Facilitator');const signups=f['Scribe / teaching points sign-ups']||'';const pres=(f.Presenter||'').trim()||(signups.match(/Case Presenter:\s*([^\n]+)/i)?.[1]||'').trim();if(!pres||/^(tbd|none|-)$/i.test(pres))gaps.push('Presenter');const scribe=(signups.match(/Scribe:\s*([^\n]+)/i)?.[1]||'').trim();if(/Scribe:/i.test(signups)&&(!scribe||/^(tbd|none|-)$/i.test(scribe)))gaps.push('Scribe');const tp=(signups.match(/Teaching Points:\s*([^\n]+)/i)?.[1]||'').trim();if(/Teaching Points:/i.test(signups)&&(!tp||/^(tbd|none|-)$/i.test(tp)))gaps.push('Teaching Points');return gaps}
