...
function calendarButton(r,t){
 ...
 if(timing.status!=='resolved')return `<span class="calendar-status muted">Calendar unavailable: ${esc(timing.reason||'confirm the source date and time')}</span>`;
 return `<span class="calendar-control"><button type="button" class="button secondary small" data-calendar="${esc(r.id)}" data-area="${esc(t)}">Add to calendar</button>${timing.durationAssumed?'<small class="muted">60-minute duration assumed</small>':''}</span>`;
}
...
function render(){
  ...
  $('#global-search').placeholder='Search all Academy records…';if(tab==='Morning Report'&&!query.trim())arrangeScheduleFilters();
}
...
function staffingSlot(r,role){
 if(!mrGaps(r).includes(role))return `<span class="matrix-slot-assigned" title="${esc(staffingRoleValue(r,role))}">${esc(staffingRoleValue(r,role)||'Not scheduled')}</span>`;
 const tier=getStaffingUrgency(SessionCore.parseDate(dateValue(r)));
 const label=tier==='urgent'?`⚠ Urgent: ${role} Needed`:tier==='upcoming'?`＋ ${role} Needed`:`＋ Claim ${role}`;
...
function matrixView(rr){
 ...
 const rows=weeks.map(w=>`<tr class="matrix-week-row"><td colspan="7">${esc(weekLabel(w.start,w.end))} · ${w.records.length} sessions</td></tr>`+w.records.map(r=>`<tr class="matrix-row"><td><div class="matrix-date-cell"><strong>${esc(recordDate(r))}</strong><span class="matrix-time-sub">${esc(SessionCore.formatSessionTime(r))}</span></div></td><td><strong>${esc(r.fields.Type||'Morning Report')}</strong></td>${['Facilitator','Presenter','Scribe','Teaching Points'].map(role=>`<td>${staffingSlot(r,role)}</td>`).join('')}<td>${sessionNotice(r)}<div class="matrix-actions">${calendarButton(r,'Morning Report')}<button class="button secondary small" data-open="${esc(r.id)}" data-area="Morning Report">Staff</button><button class="icon-button star" data-star="${esc(r.id)}" aria-label="${workspace.favorites.includes(r.id)?'Unpin':'Pin'} record">${workspace.favorites.includes(r.id)?'★':'☆'}</button></div></td></tr>`).join('')).join('');
 return `<section class="matrix-view">${scheduleSummary(rr)}<div class="matrix-card"><div class="matrix-container"><table class="matrix-table"><thead><tr>${['Date / Day','Session / Type','Facilitator','Presenter','Scribe','Teaching Points','Actions'].map(x=>`<th>${x}</th>`).join('')}</tr></thead><tbody>${rows||'<tr><td colspan="7">No dated sessions match these filters.</td></tr>'}</tbody></table></div></div>${unresolved.length?`<section class="panel unresolved-panel"><h2>Unresolved dates</h2>${unresolved.map(r=>agendaCard(r,true)).join('')}</section>`:''}</section>`;
}
...
function arrangeScheduleFilters(){
 const bar=document.querySelector('.filter-bar');if(!bar||bar.dataset.arranged)return;bar.dataset.arranged='true';bar.classList.add('schedule-filter-bar');
 ...
 for(const name of ['Upcoming','This Week','Needs Volunteers','My Sessions']){const b=document.createElement('button');b.type='button';b.className='button secondary small';b.textContent=name;b.setAttribute('aria-pressed',String(filter===name));if(name==='My Sessions'&&!workspace.reporterName){b.disabled=true;b.title='Claim a role with your name to enable My Sessions';}b.onclick=()=>{filter=filter===name?'All':name;page=0;render()};chips.append(b);}
 bar.append(chips,details);if(views)bar.append(views);if(mode==='cards'||mode==='table'){const health=document.createElement('div');health.className='schedule-health';health.innerHTML=staffingHealthBadge(records('Morning Report'));bar.after(health);}
}

window.matchMedia('(max-width:760px)').addEventListener('change',()=>{if(tab==='Morning Report'&&!query.trim())render()});
