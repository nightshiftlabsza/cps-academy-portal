...
(async()=>{
 ...
 try{
  ...
  for(const width of [1440,390]){
   ...
   const open=async id=>{await p.locator("#page h1").waitFor();const direct=p.locator(`[data-open="${id}"][data-area]`).first();if(await direct.count())await direct.click();else{await p.locator(`[data-record-menu="${id}"]`).first().click();await p.locator("#menu-act-open").click();}await p.locator('#detail-dialog[open]').waitFor();if(await p.locator('#edit-record-btn').isVisible())await p.locator('#edit-record-btn').click();};
   ...
   await nav('Morning Report');await p.locator('.schedule-secondary-filters').evaluate(el=>el.open=true);await p.locator('#scope-select').selectOption('All');
   await p.locator('#panel-date-from').fill('2026-10-26');await p.locator('#panel-date-to').fill('2026-10-26');
   const targetRecord=await p.evaluate(()=>db['Morning Report'].records.find(r=>r.fields.Date==='2026-10-26'));const children=core.splitMorningReport(targetRecord);assert.equal(children.length,2);
   ...
   await p.locator('.schedule-secondary-filters').evaluate(el=>el.open=false);
   ...
   await p.locator('.schedule-secondary-filters').evaluate(el=>el.open=true);await p.locator('#secondary-clear-btn').click();await p.locator('#scope-select').selectOption('All');
   ...
   await p.locator('#secondary-filters-done-btn').click();
