...
(async()=>{
 ...
 try{
  ...
  for(const width of [1440,390]){
   ...
   await nav('Morning Report');await p.locator('#filter').selectOption('All');
   ...
   await p.reload();await p.locator('#filter').selectOption('All');await p.locator('#date-from').fill('2026-10-26');await p.locator('#date-to').fill('2026-10-26');
   ...
   await p.locator('#clear').click();await p.locator('#filter').selectOption('All');
