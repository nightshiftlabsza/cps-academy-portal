...
(async()=>{
 ...
 try{
  ...
  for(const width of [1440,390]){
   ...
   await p.locator('.schedule-secondary-filters').evaluate(el=>el.open=true);await p.locator('#filter').selectOption('All');await p.locator('[data-open][data-area]').first().click();
