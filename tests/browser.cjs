...
(async()=>{
 ...
 try{
  ...
  for(const width of [1440,390]){
   ...
   if(await p.locator('#mr-filter-toggle-btn').count())await p.locator('#mr-filter-toggle-btn').click();
   await p.locator('.schedule-secondary-filters').evaluate(el=>el.open=true);await p.locator('#scope-select').selectOption('All');await p.locator('[data-record-menu]').first().click();await p.locator('#menu-act-open').click();
   ...
   await p.goto(base+'/#CPS%20Academy%20VMRs');await p.getByRole('link',{name:'Watch recording ↗'}).first().waitFor();
