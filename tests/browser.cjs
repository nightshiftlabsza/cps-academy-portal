...
(async()=>{
 ...
 try{
  ...
  for(const width of [1440,390]){
   ...
   if(width<=760)await p.locator('#area-picker').selectOption('CPS Academy VMRs');else await p.locator('[data-go="CPS Academy VMRs"]').click();await p.getByRole('link',{name:'Watch recording ↗'}).first().waitFor();
